...
| Dial theme | The first theme with markup of its own: an SVG clock face | Sixty ticks and twelve numerals are generated by `app.js` on first use rather than written out in `index.html`. The hands are rotated once a second by attribute, not by CSS animation, so the reduced-motion catch-all — which collapses every animation to 0.01ms — can't turn a 60-second sweep into a blur. The card is sized off the same variable as the dial so the two read as a pair. |
| Settings surface | A window, not a growing tray menu | Themes and a font list don't fit a context menu. The tray carries Open, Settings, the playback-source switch and Quit; everything else moved into the window. |
| Second playback source | Spotify Web API as an opt-in provider behind the same `state` / `send()` contract as the bridge | `main.js` holds one `provider` and swaps it; the renderer never learns which is active. The bridge is a PowerShell process, so switching to Spotify actually stops it rather than leaving it polling. System stays the zero-setup default. |
| Spotify auth | OAuth Authorization Code + PKCE, loopback redirect on a fixed port, user's own client ID | No client secret exists, so nothing to leak from the repo. Spotify matches redirect URIs exactly including the port, so the loopback port cannot be random; 48273 is registered once in the dashboard. Only the three scopes the UI uses. |
| Refresh token storage | `%APPDATA%\spotifs\spotify-auth.json`, not settings.json | It is a credential; keeping it out of the file people copy around and paste into bug reports is the point. Rotated on every refresh, since PKCE refreshes can return a new one. |
| Web API cadence | 1s on screen, 5s hidden, one confirming poll 350ms after any command | No push channel exists. The bridge's 220ms is a local WinRT read; hitting a rate-limited HTTP API that fast would earn 429s. Position is extrapolated in the renderer either way. |
| Artwork by source | System: always iTunes-upgraded. Spotify: 640px straight from Spotify | The hi-res toggle is gone. A 300px thumbnail at full screen is never what anyone wants, and the Spotify source already provides a decent cover, so there was no choice left to offer. |
...
└── src/
    ...
    │   ├── spotify.js            Web API provider: PKCE auth, token refresh, polling
...
- Tray icon with Open, Settings, a Playback source switch, and Quit. Settings
  persist to disk.
- Two playback sources. **System** (default): the SMTC bridge, artwork always
  upgraded through iTunes. **Spotify account** (opt-in): OAuth PKCE sign-in in
  the system browser, loopback redirect on `127.0.0.1:48273`, refresh token in a
  separate file, silent renewal, `/me/player` polled at 1s / 5s, commands mapped
  to the play/pause/next/previous/seek endpoints.
...
- Four themes, switchable live from Settings: **Classic** (big cover,
  ...
  date left, one wide glass card right, after the iPad lock screen), and
  **Dial** (analogue clock with the date under it left, portrait card right,
  after the Apple Watch). Same elements, rearranged by `data-theme` — Dial
  adds the clock face, which is the only theme-specific markup in the app.
- Four backgrounds for every theme, switchable live: the cover zoomed to
...
- The Spotify provider, against a fake token endpoint and a fake `/me/player`:
  25 checks. PKCE challenge matches the RFC 7636 test vector; a redirect with the
  wrong `state` gets a 400 and does not abort the flow; the real redirect
  exchanges the code with `code_verifier` and no secret; the loopback server
  closes afterwards; the refresh token lands in its own file and is rotated on
  refresh; `/me/player` maps to exactly the fields `smtc.js` emits; covers are
  downloaded to the cache and re-emitted; commands hit the right endpoints; 204
  yields the empty state; hidden backs off to 5s.
...
- Themes 2, 3 and 4 on the machine: they have been rendered headless, not run.
  The Dial theme's second hand was seen to advance between two captures, so
  the tick is running, but a full minute of it hasn't been watched.
...
- The Spotify sign-in against Spotify itself. Everything up to the real
  `/authorize` page is exercised; the consent page and a genuine token are not.
...
**The Web API port** — done, as the opt-in Spotify account source. What it
still doesn't use, though the scopes now cover it: volume, shuffle and repeat
state, and the queue. Each is a small addition to `spotify.js` and a UI decision
about where it goes.
